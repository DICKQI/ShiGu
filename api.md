...
| `short_name`| Char(50)              | 主简称/搜索关键字，如：`星铁`（通常用于主要展示/检索） |
...
| `search`     | string | 轻量模糊搜索：会同时在 `Goods.name`、`IP.name`、`IP.short_name`、`IPKeyword.value` 上匹配    |
...
1. **初始化加载**
   ...
   - 可选：预加载 IP / 角色 / 品类列表（后端目前未单独提供接口，可后续补充）。
...
2. **云展柜列表页**
   ...
   - 列表 Item 展示：
     - 主图：`main_photo`
     - 标题：`name`
     - 副标题：`ip.short_name + character.name + category.name`
     - 位置：`location_path`
